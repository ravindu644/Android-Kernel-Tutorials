...
#
# Linux 4.9 arm64 kernel build script.
# Toolchain: Proton Clang 12 + Linaro GCC 7.5 (4.9 predates LLVM=1, so a real GCC
# cross compiler is mandatory here)
#
# Put this in your kernel root, edit the settings below, then run:
#   chmod +x build_4.9.sh && ./build_4.9.sh
...
CLANG="${HOME}/toolchains/proton-12"
GCC="${HOME}/toolchains/aarch64-linaro-7.5"
cd "${KERNEL_ROOT}"
...
if [ ! -d "${CLANG}" ]; then
    ...
    git clone --depth=1 https://github.com/ravindu644/proton-12.git "${CLANG}" \
        || { rm -rf "${CLANG}"; die "Failed to clone Proton Clang 12"; }
fi
fetch "${GCC}" "https://kali.download/nethunter-images/toolchains/linaro-aarch64-7.5.tar.xz" 1
